register_alert_showcase <- function(input, output, session) {
  ...
  output$showcase_alert_api_table <- shiny::renderTable({
    data.frame(
      Argument = c("title", "...", "description", "icon", "variant", "class", "style"),
      Type = c("character | tag", "named tags", "character | tag", "character | tag", "character", "character", "character | named list"),
      Default = c("required", "none", "NULL", "\"info\"", "\"default\"", "NULL", "NULL"),
      Description = c(
