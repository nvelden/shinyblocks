...
**You are picking up the custom docs site for the `shinyblocks` R package.**

Phase 0 (kill pkgdown), Phase 1 (Next.js scaffold), Phases 2–6
(static previews, landing gallery, component index/detail pages, and
changelog), and Phase 7 (GitHub Actions) are **already done**. Treat the
phase walkthrough below as historical implementation context, not an active
to-do list.

Current details:
- Static component examples are generated by `scripts/generate-previews.R`.
- Each component detail page renders its R recipe through generated
  `block_code()` HTML (`codeHtml` in `lib/preview-manifest.json`) rather
  than raw `<pre><code>` markup.
- The local static preview runs at `http://localhost:4173/shinyblocks/`
  via `npm run preview`.
