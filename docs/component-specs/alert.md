...
- **success / warning / info** — shinyblocks feedback-state extensions using
  matching semantic surface, foreground, and border tokens.
...
| `variant` | One of `default`, `destructive`, `success`, `warning`, or `info`. |
...
| Feedback text | matching `-foreground` token |
| Feedback border | matching `-border` token |
...
- Upstream only ships `default` and `destructive` variants. The
  `success` / `warning` / `info` variants are deliberate shinyblocks
  extensions for dashboard feedback states.
