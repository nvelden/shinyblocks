...
- **success / warning / info** — shinyblocks feedback-state extensions using
  matching surface, foreground, and border tokens.
...
| `variant` | One of `default`, `secondary`, `outline`, `destructive`, `success`, `warning`, `info`, `ghost`, or `link`. |
...
| Feedback surfaces | `--success` / `--warning` / `--info` |
