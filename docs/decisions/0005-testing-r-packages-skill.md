...
`shinyblocks` needs focused, idiomatic tests as its public R API and UI helpers grow. The broader R package development skill covers package mechanics, while test design needs more specific guidance for `testthat` 3, fixtures, snapshots, mocking, cleanup, and test organization.
