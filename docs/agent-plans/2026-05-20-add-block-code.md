...
This plan details the addition of a premium visual `block_code()` component inspired by shadcn/ui.

## Goals
- Add the `block_code()` R export representing a pre-formatted, token-driven, terminal-styled code block.
...
- Implement sleek typography, terminal dots, and light/dark theme variables under `frontend/src/styles/runtime.css`.
...
- **`frontend/src/styles/runtime.css`**: Add styles for `.sb-code-block` (pre, code, header dots, copy button, hovering and dark mode Normalizations).
