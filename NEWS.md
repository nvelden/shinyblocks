...
* `block_select()` gained R-side support for `multiple = TRUE`, including vector `selected` values, `character(0)` defaults, hidden native `<select multiple>` mirrors, and an optional `max_items` payload for the upcoming runtime UI (issue #64).
