...
* Aligned playground controls panel height and scroll behavior (issue #32) across the local Shiny showcase and the docs site interactive Shinylive playgrounds. Added `showcase-playground`, `showcase-playground__controls`, and `showcase-playground__main` classes to `showcase_playground_layout()`. Absolute-positioned the controls panel (1st child) on the left to align its height with the normally-flowing preview/UI stack (2nd child) on the right, providing high-fidelity scrollbar support when the controls panel overflows. Appended showcase custom playground styles to `shinyblocks-runtime-override.css` during the docs playgrounds prebuild step so that all 27 embedded Shinylive playgrounds automatically gain identical height-alignment and scroll behavior.
