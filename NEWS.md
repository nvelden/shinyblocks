...
* Added premium visual presentational `block_code()` component with terminal-style window controls, optional language badge, and copy-to-clipboard interactions.
...
* Removed dead post-migration legacy CSS. Alert, value-box, separator, skeleton, spinner, empty, checkbox-indicator, switch-track, and Selectize selectors have been deleted from `inst/www/src/shinyblocks.css` since their components are runtime-rendered. The `.sb-button*` family is still used by `block_dark_mode_toggle()` so its rules remain. Surfaced a missing `position: relative` on the runtime `.sb-alert` rule that the legacy CSS had been silently providing.
