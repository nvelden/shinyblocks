...
* Fixed docs-site playgrounds not following the website's light/dark toggle. Shinylive runs each Shiny app inside a nested `srcdoc` iframe, so the host setting `data-theme` on the outer document never reached the app. Added an embedded-theme bridge: when running inside a frame, `shinyblocks.js` announces readiness to the top window and applies any `shinyblocks:set-theme` message; the docs `PlaygroundFrame` replies with the current theme on readiness and re-pushes it on every toggle. Top-level apps (the local showcase, real Shiny apps) are unaffected — the bridge only activates when embedded.
