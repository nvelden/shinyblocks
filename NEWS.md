...
* `block_select()` gained `multiple = TRUE`, matching Shiny's `selectInput(multiple = TRUE)` value semantics (`input$<id>` is a length-1 string in single mode, a character vector in multiple mode). The trigger shows selected items as wrapping removable chips and the popup is a checkable, multi-selectable listbox that stays open on toggle, with an optional `max_items` cap and `character(0)` defaults. Chips and the multi trigger are fully token-driven for light/dark and `block_theme()` parity (issue #64).
